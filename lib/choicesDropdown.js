...
export default class ChoicesDropdown {
   ...
   * */
  addHoveredValue () {
    const targetNode = this.searchOptionsNode(this.hoveredKey)
   ...
   * */
  removeValue (key) {
    const el = this.searchValuesNode(key)
  ...
  hoverOption (targetNode) {
    ...
    if (this.hoveredKey) {
      removeClass(
        this.searchOptionsNode(this.hoveredKey),
        hoveredClass
      )
    }
   ...
   * @return {HTMLElement}
   */
  searchOptionsNode = (key) => this.searchNode(this.nodes.optionsNode, key)

  searchValuesNode = (key) => this.searchNode(this.nodes.valuesNode, key)

  searchNode = (library, key) => (
    library.querySelector(`[${KEY_ATTRIBUTE_NAME}="${key}"]`)
  )
   ...
   * @return {HTMLElement}
   * */
  predictSelectedNode (direction, ignoreMissing = !1) {
    const hoveredNode = this.searchOptionsNode(this.hoveredKey)
    let targetNode
  ...
  removeOption (key) {
    ...
    const el = this.searchOptionsNode(key)
    if (!el) return
